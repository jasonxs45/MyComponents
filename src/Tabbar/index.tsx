...
export default class Tabbar extends React.Component<TabBarProps> {
  ...
  clickHandler: (param: number) => React.MouseEventHandler<HTMLDivElement> | undefined = (val: number): any =>  {
    this.setState({
      active: val
    })
  ...
  renderBadge = (badge: any) => {
    ...
    if (typeof badge === 'string') {
      res = badge.trim() === ''
            ? dot
            : <span className={style.tabbarItemBadge}>{badge}</span>
    }
    return res
  }
          ...
          list.map((item, index) => (
            ...
            >
              <div className={style.tabbarItemWrapper}>
                <div
                  className={style.tabbarItemIcon}
                >
                  {
                    item.customIcon
                    ? item.customIcon
              ...
              <p className={style.tabbarItemText}>
                { item.text }
              </p>
            </div>
          ))
